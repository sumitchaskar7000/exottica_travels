...
export const RequestQuoteForm = () => {
  ...
  const onSubmit = async (values: QuoteFormValues) => {
    try {
      ...
      await axios.post(`${baseUrl}/enquiries/quote`, values);
      setStatus("success");
      reset();
    } catch (e) {
      console.error(e);
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div>
          <label className="text-xs text-slate-600">Preferred destination</label>
          <input
            className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
        ...
        <div>
          <label className="text-xs text-slate-600">Departure date</label>
          <input
            type="date"
            className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
        ...
        <div>
          <label className="text-xs text-slate-600">Travellers</label>
          <input
            type="number"
            min={1}
            className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
      ...
      <div>
        ...
        <textarea
          ...
          className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
          {...register("message", {
            required: "A short trip description helps us build a quote"
          })}
        />
        {errors.message && (
          <p className="mt-1 text-xs text-red-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3">
        ...
        >
          {isSubmitting ? "Sending details..." : "Request quote"}
        </button>
        {status === "success" && (
          <p className="text-xs text-emerald-600">
            Thanks — we&apos;ll send a quote to your inbox.
          </p>
        )}
        {status === "error" && (
